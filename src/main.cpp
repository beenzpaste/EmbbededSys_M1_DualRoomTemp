...
void globalState(float T1, float T2) {
  ...
  bool both_low  = (T1 < OPT_TEMP-1.0f) && (T2 < OPT_TEMP-1.0f);    // both below optimal temp
  bool either_low   = (T1 < OPT_TEMP-0.5f) || (T2 < OPT_TEMP-0.5f); // at least one below optimal temp
  bool both_high = (T1 > OPT_TEMP+0.5f) && (T2 > OPT_TEMP+0.5f);    // both above optimal temp
    
  // state machine logic based on prev stage and current temps
  switch (stage) {
    case 0: // when both heaters are off,
      if (either_low) {
        stage = 1; // move to stage 1
      }
      else if (both_low) {
    ...
    case 1: // when already at stage 1 low heat,
      if (both_low) {
        stage = 2; // move to stage 2
      } else if (both_high) {
        stage = 0; // move to stage 0
      }
      break;

    case 2: // when at stage 2 high heat,
      if (both_high) {
        stage = 0; // move to stage 0
      } else if (!either_low) {
        stage = 1; // move to stage 1
